...
// v1.0.7 起「档案系统」默认提示词新增记忆质量标准（自包含 / 时间锚点 / 事件+意义 / 去重合并）；
// 旧默认文本（未自定义）在 getSettings 里逐字比对后自动迁移。
...
// v1.0.7 起「角色扮演」默认提示词输入说明补充 memory 消费指令（记忆分节是角色记得的经历，扮演时自然引用）；
// 旧默认文本（未自定义）在 getSettings 里逐字比对后自动迁移。
...
function getSettings(ctx) {
  ...
  if (!prompts || typeof prompts !== 'object' || Array.isArray(prompts)) {
    ...
    for (const [key, value] of Object.entries(DEFAULT_PROMPTS)) {
      ...
      && (prompts[key] === LEGACY_DEFAULT_ROLEPLAY_SYSTEM || prompts[key] === LEGACY_DEFAULT_ROLEPLAY_SYSTEM_V2 || prompts[key] === LEGACY_DEFAULT_ROLEPLAY_SYSTEM_V3 || prompts[key] === LEGACY_DEFAULT_ROLEPLAY_SYSTEM_V4)) {
      ...
      // v1.0.7 起「档案系统」默认提示词新增记忆质量标准（自包含 / 时间锚点 / 事件+意义 / 去重合并）；
      // 只有未自定义过（与任一旧版默认逐字一致）才自动升级。
      prompts[key] = value;
      shouldSave = true;
      console.warn(`[${MODULE_NAME}] 提示词「档案系统」已升级为 v1.0.7 新默认（记忆质量标准强化）`);
  ...
  const messages = [
    ...
    }] : []),
    {
      ...
      content: [
        ...
        '记忆条目必须满足 memory 质量标准：自包含（本人用「我」、他人用全名）、带时间锚点、事件+意义、一条一件事。',
        '',
