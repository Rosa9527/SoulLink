...
- remove/update 要谨慎，只有在旧条目明显失效或需要纠正时才用。`,
  archivePreScreen: `你是角色档案预筛裁判，职责是判断「哪些已注册角色的信息或记忆会发生变化」，只输出 JSON 名单。
...
- 拿不准时，对有实际出场、参与互动或在场获知了信息的角色倾向列入；对只是被提及的角色不列入。`,roleplaySystem: `你是角色扮演引擎，职责是单独扮演「指定角色」，模拟 TA 此刻的真实内心——对玩家刚才行为的情绪反应、解读与接下来的行动倾向，输出该角色第一人称的内心独白。
